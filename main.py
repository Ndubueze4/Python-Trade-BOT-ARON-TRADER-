...
GROWTH_SYMBOL = "Crash 1000 Index"
GROWTH_MIN_BAL = 50.0
GROWTH_MAX_BAL = 900.0
GROWTH_LOT = 25.0 # You can adjust this or make it dynamic
...
def sanitize_stops(symbol, order_type, price, sl, tp):
    ...
    # 4. Final Rounding to correct digits (Crucial for Synthetic Indices)
...
def calculate_dynamic_lot(symbol, risk_percent=0.01):
    """Calculates a lot size that is 'not too small' but based on equity."""
    acc = mt5.account_info()
    s_info = mt5.symbol_info(symbol)
    if not acc or not s_info:
        return MIN_LOT

    # We base the 'small' start on a 1% risk of equity spread over a standard distance
    # For Synthetic Indices, we often use a fixed denominator if SL isn't defined yet
    equity = acc.equity
    
    # Logic: $100 equity -> ~0.10 lot on Step, $1000 -> ~1.0 lot
    # Adjust the divisor (1000) to make the "small" start bigger or smaller
...
def close_partial_with_filling(symbol, position, volume, magic):
    ...
    request = {
        ...
        "type_filling": mt5.ORDER_FILLING_IOC, # Change to FOK if IOC fails
    }
...
def execute_crash_scaling(symbol, magic, analysis, acc):
    ...
    # Move price to the top so it's always accessible for management
    price = analysis['live_price']

    # --- SETTINGS FOR TRAILING STOP ---
    # Trailing distance: how far behind price the SL follows (e.g., 0.5% of price)
    # Trailing step: minimum price move required to update the SL
    trail_dist = price * 0.005  
    trail_step = price * 0.001  

    # --- VOLATILITY 50 (1S) H1 STRUCTURE + EXPANSION + 55 TRADE LIMIT ---
    if symbol == "Volatility 50 (1s) Index":
        ...
        if trade_count >= 55:
            # Skip entry logic if 55 trades are reached, but allow management of open positions
            can_enter = False
        else:
            can_enter = True

        # 2. H1 REFERENCE ANALYSIS
        rates_h1 = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_H1, 1, 1)
        if rates_h1 is None or len(rates_h1) == 0: 
            return False
            
        prev_h1 = rates_h1[0]
        h1_high, h1_low = prev_h1['high'], prev_h1['low']
        h1_mid = (h1_high + h1_low) / 2

        # --- RANGE FILTER: Detect if H1 is ranging ---
        hist_h1 = mt5.copy_rates_from_pos(symbol, mt5.TIMEFRAME_H1, 1, 10)
        if hist_h1 is not None and len(hist_h1) > 0:
            avg_h1_vol = sum(x['high'] - x['low'] for x in hist_h1) / len(hist_h1)
            if (h1_high - h1_low) < (avg_h1_vol * 0.6):
                can_enter = False # Market is ranging/tightening
        
        positions = mt5.positions_get(symbol=symbol, magic=magic) or []

        # 3. ENTRY LOGIC: CISD + FRACTAL / EXPANSION (Changed to M5)
        if not positions and can_enter:
            # Detect LTF CISD on M5
            ...
            if rates_m5 is not None and len(rates_m5) >= 2:
                ...
                # --- SCENARIO A: BUYING THE TOP (Expansion) ---
                if price > h1_high and curr_m5['close'] > prev_m5['open'] and prev_m5['close'] < prev_m5['open']:
                    direction = mt5.ORDER_TYPE_BUY
                    sl = h1_mid
                    tp = price + (price - sl) * 1.5

                # --- SCENARIO B: SELLING THE BOTTOM (Expansion) ---
                elif price < h1_low and curr_m5['close'] < prev_m5['open'] and prev_m5['close'] > prev_m5['open']:
                    direction = mt5.ORDER_TYPE_SELL
                    sl = h1_mid
                    tp = price - (sl - price) * 1.5

                # --- SCENARIO C: STANDARD REVERSAL (Discount/Premium) ---
                elif price < h1_mid and curr_m5['close'] > prev_m5['open'] and prev_m5['close'] < prev_m5['open']:
                    direction = mt5.ORDER_TYPE_BUY
                    sl = h1_low - (h1_high - h1_low) * 0.05
                    tp = price + (price - sl) * 2
                
                elif price > h1_mid and curr_m5['close'] < prev_m5['open'] and prev_m5['close'] > prev_m5['open']:
                    direction = mt5.ORDER_TYPE_SELL
                    sl = h1_high + (h1_high - h1_low) * 0.05
                    tp = price - (sl - price) * 2

                if direction is not None:
                    lot = max(0.005, round((acc.equity * 0.0001), 3))
        ...
        # 4. POSITION MANAGEMENT (Always active if positions exist)
        positions = mt5.positions_get(symbol=symbol, magic=magic) or []
        for pos in positions:
            ...
            # --- 75% Partial & BE Logic ---
            if current_gain >= (profit_dist * 0.5) and pos.volume > 0.005:
                partial_vol = round(pos.volume * 0.75, 3)
                if partial_vol >= 0.005:
        ...
        while True:
            ...
            for i, s in enumerate(SYMBOLS):
                ...
                if analysis:
                    ...
                    in_range = (10 <= acc.equity <= 5000)
                    is_scaling_symbol = s in ["Volatility 50 (1s) Index"]
                    ...
                    if in_range:
                        if is_scaling_symbol:
                            # RUN SCALING AND STOP (This function handles its own exits)
                            is_scaling = execute_crash_scaling(s, magics[i], analysis, acc)
                        else:
                            # Skip Volatility indices if we are in strict scaling mode
                            continue 
                    
                    # 3. FALLBACK / NORMAL LOGIC
                    # Only runs if is_scaling is False (Account > 5000 OR execute_crash_scaling returned False)
                    if not is_scaling:
                        take_partial_profits(s, magics[i])
                        manage_active_positions(s, magics[i])
                        scalp_logic(s, magics[i], analysis)

                    # 4. DASHBOARD UPDATE (Visuals only)
